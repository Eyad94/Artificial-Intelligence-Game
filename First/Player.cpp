...
//i = check_nearby_health();
i = 0;
printf("####################	i = %d	###################\n", i);
*this->maze[50][50] = HEALTH;
//this->node->setTargetPoint(new Point2D(50 , 50));
this->enemy = HEALTH;

printf("FUNC : search_health :|| priority_queue_size : %d || gray_size : %d	|| black_size %d\n", this->priority.size(), this->gray.size(), this->black.size());

A_start_algorithm();
if (this->a_star_started == false)
{
this->health_points = 100;
printf("i fount the medicament and my health : %d/100", this->health_points);
*this->a_star_started = true;
}
...
int Player::check_nearby_health()
{
int i, j , index;
...
if (distance(health_arr[i]) < distance(health_arr[i + 1]))
index = i;
else
index = i + 1;
}
...
i = check_nearby_munitions();
this->node->setTargetPoint(&munition_arr[i]);
this->enemy = MUNITIONS;

printf("FUNC : search_munitions :|| priority_queue_size : %d || gray_size : %d	|| black_size %d\n", this->priority.size(), this->gray.size(), this->black.size());

A_start_algorithm();
if (this->a_star_started == false)
{
this->munitions_points = 100;
printf("i fount the munitions and my munitions : %d/100", this->munitions_points);
*this->a_star_started = true;
}
...
int Player::check_nearby_munitions()
{
int i, j, index;
...
if (distance(munition_arr[i]) < distance(munition_arr[i + 1]))
index = i;
else
index = i + 1;
}
